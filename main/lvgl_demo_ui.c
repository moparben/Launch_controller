...
void example_lvgl_demo_ui(lv_display_t *disp)
{
    ...
    // Create a simple label test instead of complex animations
    lv_obj_t *label = lv_label_create(parent);
    lv_label_set_text(label, "ESP32-P4 + JD9365\nDisplay Working!\n\nSerial Config Ready\nType 'help' in console");
    lv_obj_set_style_text_color(label, lv_color_white(), 0);
    lv_obj_set_style_text_font(label, &lv_font_montserrat_16, 0);
    lv_obj_center(label);
    
    // Create colored rectangles to test display regions  
    lv_obj_t *rect1 = lv_obj_create(parent);
    lv_obj_set_size(rect1, 150, 80);
    lv_obj_set_pos(rect1, 50, 200);
    lv_obj_set_style_bg_color(rect1, lv_palette_main(LV_PALETTE_RED), 0);
    lv_obj_set_style_border_width(rect1, 0, 0);
    
    lv_obj_t *rect2 = lv_obj_create(parent);
    lv_obj_set_size(rect2, 150, 80);
    lv_obj_set_pos(rect2, 225, 200);
    lv_obj_set_style_bg_color(rect2, lv_palette_main(LV_PALETTE_GREEN), 0);
    lv_obj_set_style_border_width(rect2, 0, 0);
    
    lv_obj_t *rect3 = lv_obj_create(parent);
    lv_obj_set_size(rect3, 150, 80);
    lv_obj_set_pos(rect3, 400, 200);
    lv_obj_set_style_bg_color(rect3, lv_palette_main(LV_PALETTE_BLUE), 0);
    lv_obj_set_style_border_width(rect3, 0, 0);

    return; // Skip complex animations that cause crashes
